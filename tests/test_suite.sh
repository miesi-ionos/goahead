...
# Test 6: Positive test with actual client execution
test_positive_execution() {
    log_info "Running Test 6: Positive execution test with actual client"

    local fqdn="simple-positive01.test.tld"
    local client_id="positive01"
    local cluster="simple-cluster"
    ...
    log_info "Step 1: Running goahead_client to request restart approval..."

    # Run client in background and capture output
    local client_output=""
    local client_response=""

    cd "$CLIENT_DIR"
    timeout 30s ./goahead_client -config "$CONFIG_DIR/clients/client_${client_id}.yml" -debug > "$RESULTS_DIR/client_${client_id}_output.log" 2>&1 &
    local client_pid=$!
    ...
    if [[ $client_exit_code -eq 0 ]] || [[ $client_exit_code -eq 124 ]]; then
        # Parse client output to check response
        client_response=$(grep -o '"go_ahead":[^,}]*' "$RESULTS_DIR/client_${client_id}_output.log" 2>/dev/null || echo "")

        if [[ "$client_response" =~ "true" ]]; then
            test_result "PASS" "Positive test - Client received go_ahead:true"
        else
            test_result "FAIL" "Positive test - Client should receive go_ahead:true"
            log_info "Client response: $client_response"
        fi
    else
        test_result "FAIL" "Positive test - Client execution failed"
        cat "$RESULTS_DIR/client_${client_id}_output.log" 2>/dev/null || echo "No client output available"
    fi

    # Validate goahead action was called
    sleep 2
    if validate_action_called "goahead" "$fqdn" "$cluster"; then
        test_result "PASS" "Positive test - Goahead action executed"
    else
        test_result "FAIL" "Positive test - Goahead action should be executed"
    fi

    # Simulate server coming back online quickly
    log_info "Step 2: Simulating server coming back online..."
    touch "$RESULTS_DIR/${fqdn}-server-online"

    # Wait for completion actions
    sleep 5

    if validate_action_called "completion" "$fqdn" "$cluster"; then
        test_result "PASS" "Positive test - Completion action executed"
    else
        test_result "FAIL" "Positive test - Completion action should be executed"
    fi

    # Show action logs for verification
    log_info "Action execution logs:"
    if [[ -f "$RESULTS_DIR/reboot_goahead_actions.log" ]]; then
        echo "  Goahead actions: $(wc -l < "$RESULTS_DIR/reboot_goahead_actions.log") entries"
    fi
    if [[ -f "$RESULTS_DIR/reboot_completion_actions.log" ]]; then
        echo "  Completion actions: $(wc -l < "$RESULTS_DIR/reboot_completion_actions.log") entries"
    fi
}
